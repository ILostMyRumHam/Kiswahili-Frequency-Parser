    ...
    for i in splitter:
        if i:
            ...
            try:
                ...
                swahili_word = trim_quotation_marks.replace("'","").replace("*", "").replace(":", "")
            except IndexError:
                swahili_word = "Word not found"
            try:
                english_possible_definition = split_up[split_up.index("{")+1].replace("*", "").replace(":", "")
            except ValueError:
                english_possible_definition = "English Equivalent not found"
            try:
                word_class = word_classes_reference.get(split_up[2]).replace("*", "").replace(":", "") if word_classes_reference.get(split_up[2]) != None else "Word Class not found"
            except IndexError:
